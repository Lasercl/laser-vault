...
const AiAdvisor = ({ back }) => {
  ...
  const fetchAiAdvice = async () => {
    try {
      ...
      const prompt = `Laser's Data: Cash Rp${data.cash}, Assets Rp${totalAssets}, Ratio ${cashRatio.toFixed(1)}%. Give a quick professional diagnosis.`;
  ...
  const handleConsultation = async () => {
    ...
    try {
      ...
      const prompt = `
        Context Keuangan Laser: Cash Rp${data.cash}, Total Aset Rp${totalAssets}.
  ...
  return (
    <>
      ...
      <div style={{ ...cardStyle, background: 'linear-gradient(135deg, #1e293b 0%, #0d1117 100%)', marginBottom: '20px' }}>
        <p style={{ fontSize: '8px', fontWeight: '900', color: '#8b949e', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '10px' }}>{t('currentDiagnosis')}</p>
      ...
      <div style={{ ...cardStyle, marginBottom: '20px' }}>
        <p style={{ fontSize: '8px', fontWeight: '900', color: '#3b82f6', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '15px' }}>{t('askNeuralEngine')}</p>
        <textarea
        ...
        >
          {isConsulting ? t('syncing') : t('executeConsultation')}
        </button>
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
          <span style={{ fontSize: '9px', fontWeight: '900', color: '#8b949e' }}>{t('liquidityRatio')}</span>
